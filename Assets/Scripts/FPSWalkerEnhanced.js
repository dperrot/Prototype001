...
private var soundCheck : float;
private var isFrozen = true;

function SetFrozen(b : boolean) { isFrozen = b; }
...
function FixedUpdate() {
	...
	// If we are moving, increase sound check, else turn it off
	if (inputX || inputY) {
		soundCheck += Time.deltaTime;
		if (soundCheck >= walkAlternate) {
			audio.Play();
			soundCheck = 0;
		}
	} else {
